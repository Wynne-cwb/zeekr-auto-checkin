...
你只需要提供一次 token，剩下的交给 openclaw。

[效果示例](#效果示例) · [抓包](#抓包-token) · [安装](#安装) · [故障排查](#故障排查)
...
```
$ ZEEKR_TOKEN="Bearer eyJ..." node scripts/checkin.mjs
...
openclaw 帮你跑签到，只需要从极氪 App 的网络请求里抓一个 token。五步搞定：
...
5. **复制任意一条请求的 `Authorization` 请求头**（`Bearer eyJ...` 整串）

把这个值丢给 openclaw，剩下的它来搞定。
...
```bash
node scripts/checkin.mjs --token "Bearer eyJ..."
```
...
- 本项目不会存储、上传或转发你的 Token，凭据只存在于你本地的 launchd plist 和脚本环境变量里
- 不对因使用本项目导致的任何账号异常负责
