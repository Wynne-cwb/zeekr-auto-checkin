...
1. **Bearer Token**
   > 在极氪 App（iOS / Android）里用抓包工具（Stream / Charles / Proxyman / HttpCanary 等）抓请求，**按域名 `api-gw-toc.zeekrlife.com` 过滤**，从任意一条请求的请求头里复制 `Authorization` 的完整值（以 `Bearer eyJ...` 开头）。

2. **device_id**：同一条请求的请求头里的 `device_id`（长数字字符串），与 App 实际请求一致。
...
流程：装好工具和 HTTPS 证书 → 打开极氪 App 随便点一下 → 过滤域名 `api-gw-toc.zeekrlife.com` → 从任意一条请求的请求头里复制 `Authorization` 和 `device_id`。
...
- `EnvironmentVariables` 同时设置 **`ZEEKR_TOKEN`** / **`ZEEKR_DEVICE_ID`** / **`NOTIFY_CHANNEL`** / **`NOTIFY_TARGET`**
...
关注 `event triggers` 的 `Hour` / `Minute`、`environment` 里是否有 `ZEEKR_TOKEN` / `ZEEKR_DEVICE_ID` / `NOTIFY_CHANNEL` / `NOTIFY_TARGET`、`last exit code`、`runs`。
