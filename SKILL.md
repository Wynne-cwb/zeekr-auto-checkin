...
2. **领取循环**：最多 **5 轮**（常量 `MAX_UNCOLLECT_ROUNDS`）。每轮顺序为：
   - 调用 `getUncollected` 查询可领列表；
   - 若碎片与步行均为 **0 条**，则 **本循环结束**（首轮无奖励会打「暂无可领取奖励」，后续轮无奖励会打「本轮无可领取，结束」）；
   - 否则：`claimDebris`（每个碎片 **单独请求**）→ 随机 **2–3s** → `claimWalk`（每个步行奖励 **单独请求**）；
   - 同一轮内，相邻两次领取接口之间随机 **1–2s**；
   - 若本轮不是第 5 轮，本轮结束后再随机 **2–3s** 再进入下一轮查询。
...
- **延迟**：签到后 4–5s；每轮内碎片与步行之间 2–3s；轮与轮之间 2–3s；同一轮内连续两次领取接口之间 1–2s。
- **多轮查询**：`getUncollected` 无奖励时提前结束循环，不必跑满 5 轮。
